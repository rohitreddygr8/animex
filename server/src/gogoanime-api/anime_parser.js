...
export const scrapeDownloadLinks = async ({ episodeId }) => {
  ...
  if (cachedDownloadLinks[episodeId]) {
    if (
      parseInt(new URL(cachedDownloadLinks[episodeId].sources[0].url).searchParams.get("expires")) <
      Date.now() + 1.08e7
    ) {
  ...
  try {
    ...
    const d_links = await d_page.$$eval("div.mirror_link:nth-child(1) > div.dowload", (links) =>
      links.map((link) => {
        return {
