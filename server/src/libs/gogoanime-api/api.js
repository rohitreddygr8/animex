...
app.get("/download", async (req, res) => {
  try {
    const downloadLink = req.rawHeaders.find((x) => x.includes("https://") && x.includes(".mp4"));

    if (!downloadLink) {
      return res
        .status(400)
        .json({ error: "No downloadLink provided. Make sure to add the downloadLink in the headers." });
    }

    await axios.get(downloadLink, { headers: { Referer: DownloadReferer }, responseType: "stream" }).then((stream) => {
